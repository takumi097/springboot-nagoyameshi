...
public class AdminTermControllerTest {
	...
	public void 管理者としてログイン済の場合は管理者用の利用規約ページが正しく表示される() throws Exception {
		mockMvc.perform(get("/admin/terms"))
	...
	public void 未ログインの場合は利用規約を更新せずにログインページにリダイレクトする() throws Exception {
		...
		String originalTerm = term.getContent(); 
		...
		assertThat(term.getContent()).isEqualTo(originalTerm);
	}
	
	@Test
	@WithUserDetails("taro.samurai@example.com")
	@Transactional
	public void 一般ユーザーとしてログイン済の場合は利用規約を更新せずに403エラーが発生する() throws Exception {
		Term term = termService.findFirstTermByOrderByIdDesc();
		String originalTerm = term.getContent();
	...
	@WithUserDetails("hanako.samurai@example.com")
	@Transactional
	public void 管理者としてログイン済の場合は利用規約更新後に利用規約ページにリダイレクトする() throws Exception {
		mockMvc.perform(post("/admin/terms/update")
				.with(csrf())
				.param("content", "テスト"))
		.andExpect(status().is3xxRedirection())
		...
		Term term = termService.findFirstTermByOrderByIdDesc();
		assertThat(term.getContent()).isEqualTo("テスト");
	}
}
